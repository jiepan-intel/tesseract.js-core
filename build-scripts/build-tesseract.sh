...
#  fi
  ...
  $CMAKE_CMD .. -DCMAKE_CXX_FLAGS="$CXXFLAGS" ${CM_FLAGS[@]} -D HAVE_AVX2=1  -D HAVE_SSE4_1=1
#$MAKE_CMD -j${PROC} -n
  $MAKE_CMD -j${PROC}  VERBOSE=1
  if [ $BUILD_SINGLE = 0 ]; then
    $CMAKE_CMD .. -DCMAKE_CXX_FLAGS="$CXXFLAGS" ${CM_FLAGS[@]} -D HAVE_AVX2=1  -D HAVE_SSE4_1=1 -D DISABLED_LEGACY_ENGINE=1
    $MAKE_CMD -j${PROC}

    $CMAKE_CMD .. -DCMAKE_CXX_FLAGS="$CXXFLAGS" ${CM_FLAGS[@]} -D HAVE_SSE4_1=0 -D HAVE_AVX2=0 -D DISABLED_LEGACY_ENGINE=1
    $MAKE_CMD -j${PROC}

    $CMAKE_CMD .. -DCMAKE_CXX_FLAGS="$CXXFLAGS" ${CM_FLAGS[@]} -D HAVE_SSE4_1=0 -D HAVE_AVX2=0 -D DISABLED_LEGACY_ENGINE=0
    $MAKE_CMD -j${PROC}
  fi
